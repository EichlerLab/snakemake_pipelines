...
# print(FOFNS)
shell.prefix("source %s/../.test/env.cfg; " % (SNAKEMAKE_DIR))
...
def getNuc(wildcards):
    bam = f"{wildcards.sample}/alignments/{wildcards.tech}/{wildcards.type_map}/all_{wildcards.tech}.bam"
...
rule all_eval:
    input:
        expand("{sample}/assembly_eval/{tech}/{type_map}/final_qc.bed",
            zip,
...
rule combine_asm:
    input:
        unpack(getAsm),
    output:
        comb_fast="{sample}/asm/both_haps.fasta",
        comb_fai="{sample}/asm/both_haps.fasta.fai",
    threads: 1
    resources:
        mem=8,
        load=100,
...
rule get_depth_cov:
    input:
        ...
        bam="{sample}/alignments/{tech}/{type_map}/all_HiFi.bam",
    output:
...
rule rustybam:
    input:
        ...
        bam="{sample}/alignments/HiFi/{type_map}/all_HiFi.bam",
    output:
...
rule filter_rustybam:
    ...
    run:
        ...
        bed_df["second_highest"] = bed_df.iloc[:,][
            ["A", "G", "T", "C"]
